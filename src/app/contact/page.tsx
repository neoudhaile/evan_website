            ...
            <FadeIn delay={900} direction="right">
              <div className="space-y-4 sm:space-y-6">
                {content.socialLinks.map((link) => (
                  ...
                  >
                    <div className={`w-12 h-12 ${link.bgColor} rounded-full flex items-center justify-center`}>
                      {getSocialIcon(link.name)}
                    </div>
                    <span className="caption text-gray-300 group-hover:text-white">{link.name}</span>
                  </a>
                ))}
              </div>
            </FadeIn>
